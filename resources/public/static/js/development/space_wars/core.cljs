...
; Number  -> [[Number Number]]
(defn rand-coords [n]
  "Returns a vector of random [x y] coord pairs that fit on the canvas."
  (vec (repeatedly n
         #(vector (rand-n 0 world-width) (rand-n 0 world-height)))))
...
; [[x y]] -> Image
(defn draw-stars! [star-coords]
  (q/push-style)
  (q/fill 255)
  (let [star-size (/ world-width 160)]
    (doseq [[x y] star-coords]
      (q/ellipse x y star-size star-size)))
  (q/pop-style))
...
; [[x y]] -> [[x y]]
(defn move-stars [star-coords]
  "Moves each star's y-coordinate by offset down the canvas."
  (let [offset 0.8]
    (for [[x y] star-coords]
      [x (mod (+ offset y) world-height)])))
...
(defn init-state! []
  (let [w2 (/ world-width 2)
        n-stars 48
        star-coords (rand-coords n-stars)]
    {:score 0
     :lifes 3
     :game-state :playing
     :state-counter 0
...
; State -> State
(defn update-state [{:keys [score tank missiles bombs ufos hits star-coords lifes game-state state-counter]
                     :as state}]
  (let [bg-state (assoc state :star-coords (move-stars star-coords))]
    (cond
...
(defn draw-state [{:keys [score tank missiles bombs ufos hits star-coords lifes game-state]
                   :as state}]
    (apply q/background (:dark-blue colors))
  ;  ((qt/at 100 80 (qt/in 40 40 alien-img)))
    (draw-stars! star-coords)
